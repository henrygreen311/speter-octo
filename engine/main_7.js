...
(async () => {
  ...
  const renamedPath = path.join(__dirname, 'tempmail_7.json');
  let accounts;
  ...
  try {
    try {
      ...
      try {
        ...
        console.log('Loaded existing tempmail_7.json');
      } catch {
      ...
      accounts = JSON.parse(data);
      console.log('Copied and renamed Creator/tempmail_accounts.json ➜ tempmail_7.json');
    }
  } catch (error) {
  ...
  if (validAccounts.length === 0) {
    console.log('No accounts with register: yes found. Recopying tempmail_7.json from Creator...');
    try {
      await fs.copyFile(sourcePath, renamedPath);
      const data = await fs.readFile(renamedPath, 'utf8');
      accounts = JSON.parse(data);
      console.log('Successfully recopied Creator/tempmail_accounts.json ➜ tempmail_7.json');
      ...
      console.error('Error recopying tempmail_7.json:', copyError);
      return;
    }
  }
...
try {
  ...
  const { stdout } = await execPromise(`python3 ../Creator/tempmail.py inbox ${selectedAccount.address}`);
    ...
    let targetUrl;
    try {
      targetUrl = (await fs.readFile(path.join(__dirname, 'url_7.txt'), 'utf8')).trim();
      if (!targetUrl.startsWith('http')) throw new Error('Invalid URL format in url_7.txt');
      console.log('Loaded target URL from url_7.txt:', targetUrl);
      await page.goto(targetUrl, { waitUntil: 'load', timeout: 60000 });
    } catch (error) {
      console.error('Error reading or navigating to url_7.txt:', error.message);
      await browser.close();
      return;
    }
